...
#include "soft_i2c.h"
#include "math.h"
unsigned char INA219::begin(unsigned char address)
{
...
int INA219::readRegister16(unsigned char reg)
{
    int value;
    unsigned char temp[2] = {0};
	soft_I2C.i2c_read(inaAddress,reg,temp,2);
	value = temp[0];
	value = (value << 8) | temp[1];
    return value;
}

void INA219::writeRegister16(unsigned char reg, unsigned int val)
{
    unsigned char temp[2] = {(unsigned char)(val >> 8),(unsigned char)(val & 0xFF)};
    soft_I2C.i2c_write(inaAddress,reg,temp,2);
}
